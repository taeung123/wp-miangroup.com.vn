<?php
    ...
    query_posts( array ( 'category__in' => '24'));
    if(have_posts()):
        the_post();
        $parentid = 5;
...
<div class="cat-info">
    ...
    <div class="col-cat-info">
        <?php echo $catParent->description; ?>
    </div>
            ...
            <ul class="col<?php echo count($term_child) ?>">
                <?php 
                    foreach ($term_child as $key => $value) : 
                        ...
                        $urlname = $value->name;
                ?>
                    <li class="samecat cat<?php echo $value->term_id ?> <?php if($value->term_id == $cat_id): ?> active curmb <?php endif; ?> " style="cursor: pointer" onclick="getPostFashion( <?php echo $value->term_id ?>, '<?php echo $linkcat ?>', '<?php echo $urlname ?>')">
